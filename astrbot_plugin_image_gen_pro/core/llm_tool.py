...
from .utils import apply_template_variables
...
def _parse_preset(
    ...
    # 尝试替换模板变量 {变量名}，变量从用户额外输入的 prompt 中提取
    final_prompt, remaining = apply_template_variables(final_prompt, prompt)
    # 如果变量被提取后还有剩余文本（非 key=value 的内容），重新拼接
    if remaining:
        final_prompt = f"{final_prompt} {remaining}".strip()
